...
/*   Updated: 2022/05/14 06:42:22 by tel-mouh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	min_in_top(t_list **stack_a)
{
	...
	r = smart_rotate(stack_a, min.index < ((*stack_a)->size - 1) / 2, \
		min_content);
	return (r);
}
...
int	search_bt(t_list **stack, int *nlist, int len)
{
	...
	while (++i <= size / 2)
	{
		if (!ft_in_table(nlist, head->content, len))
			return (smart_rotate_pr(stack, 1, head->content), 1);
		else if (!ft_in_table(nlist, last->content, len))
			return (smart_rotate_pr(stack, 0, last->content), 1);
		head = head->next;
		last = last->prev;
	}
	return (0);
